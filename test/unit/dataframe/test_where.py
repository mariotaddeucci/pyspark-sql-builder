...
def test_where_condition(spark: SparkSession) -> None:
    ...
    assert df.generate_query() == ("SELECT * FROM (SELECT * FROM users) AS _t WHERE `age` > 18")


def test_multiple_where(spark: SparkSession) -> None:
    df = spark.table("users").where(F.col("age") > 18).where(F.col("status") == "active")
    expected = (
        "SELECT * FROM (SELECT * FROM (SELECT * FROM users) AS _t WHERE `age` > 18) AS _t WHERE `status` = 'active'"
    )
...
def test_select_with_where(spark: SparkSession) -> None:
    ...
    expected = "SELECT * FROM (SELECT `id`, `name`, `age` FROM users) AS _t WHERE `age` >= 21"
    assert df.generate_query() == expected
